...
### 36. 서버사이드렌더링, 클라이언트사이드렌더링
**서버사이드 렌더링 (SSR)** <br />
  > 서버사이드 렌더링`(SSR)` 은 페이지를 이동할 때마다 새로운 페이지를 요청한다.<br />
  서버 연산을 통해서 렌더링하고 완성된 페이지 형태로 응답한다.

장점<br />
- 검색엔진 최적화 `(SEO)` 가능 → 서버 사이드 렌더링을 통해 얻을 수 있는 가장 큰 장점이다.
- 성능개선 → 첫 렌더링된 `html` 을 클라이언트에게 전달해 주기때문에 초기 로딩 속도를 많이 줄여 줄 수 있다.
...
단점<br />
- 초기 구동속도가 느리다.
- 검색엔진 최적화가 어렵다. → 부분적인 서버사이드렌더링이 필요하다.
- 페이지 이동시 렌더링이 빠르고, 코드 재사용성이 높다.
- 앞서 `SSR`과 달리 클라이언트에서 렌더링 하는방식이다.
...
```
이 부분은 외부에서도 계속 쓰이고 있는 상황이기 때문에 클로저가 된 상황이고,<br/>
또한 함수가 함수를 값으로 다루기 때문에 일급함수이기도 하다.
